...
def derivar(estado, t, m1, m2, m3):
    ...
    """
    pos1 = np.array([estado[0], estado[1],estado[2]])
    pos2 = np.array([estado[6], estado[7],estado[8]])
    pos3 = np.array([estado[12], estado[13],estado[14]])
    
    vel1 = np.array([estado[3], estado[4],estado[5]])
    vel2 = np.array([estado[9], estado[10],estado[11]])
    vel3 = np.array([estado[15], estado[16],estado[17]])
    ...
    return np.array([
        vel1[0], vel1[1],vel1[2], aceleracionCuerpo1[0], aceleracionCuerpo1[1],aceleracionCuerpo1[2],
        vel2[0], vel2[1],vel2[2], aceleracionCuerpo2[0], aceleracionCuerpo2[1],aceleracionCuerpo2[2],
        vel3[0], vel3[1],vel3[2], aceleracionCuerpo3[0], aceleracionCuerpo3[1],aceleracionCuerpo3[2]
    ])
...
# Posiciones iniciales en el plano tridimensional (en metros)
posCuerpo1 = np.array([0, 0,0])   # Cuerpo 1
posCuerpo2 = np.array([2.0e11, 0,0])  # Cuerpo 2
posCuerpo3 = np.array([4.0e11, 0,0])  # Cuerpo 3

# Velocidades iniciales en el plano tridimensional (en metros por segundo)
v_inicial1 = np.array([0, 0,0])  # Cuerpo 1
v_inicial2 = np.array([0, 22000,0])  # Cuerpo 2
v_inicial3 = np.array([0, 15000,0])
...
trayectoriaCuerpo1 = estados[:, 0:3]
trayectoriaCuerpo2 = estados[:, 6:9]
trayectoriaCuerpo3 = estados[:, 12:15]
